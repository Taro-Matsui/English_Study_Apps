...
| 候補 | 内容 | 事業観点 |
|---|---|---|
| **Pro トライアル** | 期間限定でPro機能（Sonnet判定等）を開放 | 有料転換の主導線。SRSで定着体験を作った今が投入時 |
| **チャーン防衛** | 解約前の引き留め（休眠検知→復習リマインド、ダウングレード導線） | LTV防衛。SRSの due 件数を再訪フックに使える |
...
- 産出方向クイズ（日→英綴り）。前段フィルタは方向フラグで分岐できる構造を確保済み
