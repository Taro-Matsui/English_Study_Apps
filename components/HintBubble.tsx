...
export function HintBubble({
  ...
  const arrowPos =
    position === 'top'
      ? 'top-full left-1/2 -translate-x-1/2 border-t-[7px] border-t-yellow-400 border-x-[6px] border-x-transparent'
      : 'bottom-full left-1/2 -translate-x-1/2 border-b-[7px] border-b-yellow-400 border-x-[6px] border-x-transparent'
      ...
      {visible && (
        <>
          <div className={`absolute ${bubblePos} z-50 pointer-events-none`}>
            <div className="pointer-events-auto relative bg-yellow-400 text-yellow-900 text-xs font-semibold px-3.5 py-2.5 rounded-2xl shadow-xl max-w-[220px] text-center leading-relaxed">
              {message}
              <button
                onClick={(e) => { e.stopPropagation(); dismiss() }}
                className="absolute -top-2 -right-2 w-5 h-5 bg-yellow-600 text-white rounded-full text-[10px] flex items-center justify-center hover:bg-yellow-700 transition-colors"
                aria-label="閉じる"
              >
                ✕
              </button>
